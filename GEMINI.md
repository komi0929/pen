...
7. **⚠️ PowerShell 環境（厳命）** → `&&` 禁止（`;` を使う）。`tail` `head` `grep` `cat` 等の Linux コマンド禁止（PowerShell 代替を使う）
8. **⚠️ コード編集後の確認（厳命）** → import 変更後は `view_file` で保存を確認してからビルドすること。import 漏れによるビルド失敗を防ぐ
9. **⚠️ ファイル編集前の確認（厳命）** → `replace_file_content` の TargetContent は必ず `view_file` で実ファイルの該当行を確認してから指定する。推測で書かない
