...
- [ ] **⚠️ 改善履歴の記録はユーザー確認必須（厳命）** → 大規模な変更を行った場合、push前に「この変更を改善履歴に残しますか？」とユーザーに確認する。**バグ修正や軽微な変更はユーザーにとって無価値なので記録しない。** ユーザーがOKした場合のみ `POST /api/admin/improvement-history` を実行する。
